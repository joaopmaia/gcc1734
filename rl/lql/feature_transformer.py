...
from taxi_feature_extractor import FeatureExtractor

class FeatureTransformer:
  def __init__(self, env, use_polynomial_features: bool):
    self.env = env
    self.poly = None
    if use_polynomial_features:
      self.poly = PolynomialFeatures(interaction_only=True)
    self.fex = FeatureExtractor(env)
    self.__initInternal()
  ...
  def __initInternal(self):
    ...
    dataset = np.zeros([len(observation_examples) * n_actions, FeatureExtractor.NUM_FEATURES])
    ...
    for s in observation_examples:
      for a in range(n_actions):
        newrow = self.fex.getFeaturesAsArray(s,a)
        dataset[i] = newrow
        i += 1
  ...
  def get_features_as_array(self, state, action):
    feature_vector = self.fex.getFeaturesAsArray(state, action)
